...
const placeOrder = async (req, res) => {
  try {
    ...
    const shippingCharge = subtotal < 500 ? 50 : 0; // Modified
    
    const total = subtotal - discount + shippingCharge; // Modified
    ...
    // Create order
    const order = new Order({
      orderId: uuidv4(), // Added: Use UUID for orderId
      userId,
    ...
    await Cart.findOneAndUpdate(
      { userId },
      { $set: { items: [], discount: 0 } } // Modified
    );

    return res.status(200).json({ success: true, message: 'Order placed successfully', orderId: order.orderId }); // Modified
  } catch (error) {
    console.error('Error in placeOrder:', error);
    return res.status(500).json({ success: false, message: 'Order placement failed' }); // Modified
  }
};
...
const cancelOrder = async (req, res) => {
  ...
  try {
    ...
    // Refund logic (uncomment when ready)
    /*
